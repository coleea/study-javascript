...
에스트렐라는 esbuild 기반의 다용도 `빌드 툴`이다. 아래와 같은 기능을 제공한다

- 원본 파일이 변경될 때 자동으로 HMR (핫 모듈 리로딩)
- 한 번에 여러 프로젝트를 동시에 빌드할 수 있다
- TypeScript 진단이 병렬로 실행됨 (보다 빠른 타입체킹)
...
A. 바벨과 마찬가지로 트랜스파일러이다. AST를 구문분석해서 es5와 같은 구버전 자바스크립트로 트랜스파일하는 데 쓰인다\
공식 사이트에 의하면 컴파일과 번들링 모두에 사용할 수 있다고 하니 이제는 통합 빌드 툴이 된 것으로 보인다\
공식 사이트는 https://swc.rs/ 이니 참조할 것

### Q. 로마(Rome)가 뭔가 ?

로마는 린터, 컴파일러, 번들러가 합쳐진 통합 빌드툴이다. node.js기반 웹개발에 대한 거의 모든 파일을 다룬다 (js, ts, JSON, HTML, 마크다운, CSS)\
...
환경변수를 저장하는 설정 파일이다. 이 파일에서 정의한 값을 js에서 변수로 활용할 수 있다.\
예를 들어 procees.env.변수명 이런식이다\
next.js에서는 .env파일 대신에 next.config.js파일에 환경변수를 저장한다
...
### Q. 프리티어(prettier)가 무엇인가 ?

A. 코드를 정렬해주는 프로그램이다. 이러한 코드정렬 프로그램을 포멧터(formatter)라고 부른다\
개념적으로 eslint와 비슷하다
...
A. package.json을 실행하면 자동으로 생성되는 의존성 관련 파일이다\
의존 패키지의 버전 정보 및 정확한 버전은 무엇인지, 해쉬키의 값이 얼마인지, 의존성 패키지는 무엇인지 모든 정보가 다 담겨져 있다.\
왜 이런 정확한 의존성 트리가 필요할까? 실제 개발에 라이브러리 그대로의 파일을 다운로드 받기 위해서이다\
버전이 달라지면 프로그램 실행도중 예상치 못한 에러가 발생할 수 있고 프로그램 자체가 돌아가지 않기도 한다\
그러므로 정확한 버전을 다운로드 하는것은 실제 개발에서 중요하다


#### Q. package.json을 매니페스트 파일이라고 하는데 매니페스트 파일이 무슨뜻인가?

A. 매니페스트는 나타내다, 드러내 보이다 라는 뜻이다. 매니페스트 파일은 패키지를 묘사하는 파일이다\

#### Q. package.json 파일에서 "type": "module" 라고 넣는 것과 안 넣는 것과 무슨 차이인가 ?

A.`ES Module`을 사용해서 빌드하겠다는 뜻이다. 이걸 기입하지 않으면 `CommonJS`기반으로 빌드를 수행한다
